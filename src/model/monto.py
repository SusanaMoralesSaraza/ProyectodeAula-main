...
# --- Excepciones específicas (nombres claros y contexto) ---
...
class ErrorDemasiadasCuotas(Exception):
    """Error: el plazo de amortización permitido es de 10 a 15 años (120 a 180 meses)."""


# --- Funciones de negocio (con type hints y docstrings) ---
def calcular_tasa_mensual(tasa_interes_anual: float) -> float:
    """
    Convierte la tasa anual (en %) a tasa efectiva mensual.
    Ej.: 12 -> 0.01
    """
...
def calcular_valor_a_pagar(
    ...
    if plazo_amortizacion < MESES_MINIMOS_AMORTIZACION or plazo_amortizacion > MESES_MAXIMOS_AMORTIZACION:
...
def calcular_cuota_mensual(
    ...
    """
    Cuota fija (sistema francés): valor_a_pagar * [ r / (1 - (1+r)^(-n)) ].
    """
    # Nota: tasa_mensual > 0 según las validaciones previas.
